...
public class FileLoggingDBDecorator extends DBDecorator {
    static final Logger log = Logger.getLogger(FileLoggingDBDecorator.class.getName());

    public FileLoggingDBDecorator(DBStorage database) {
        super(database);
    }

    @Override
    public void createTable(String name, int columns) {
        log.debug("Table " + name + " with " + columns + " columns created");
    ...
    public DBTable select(String tabName, int col, String selector) {
        log.debug("Selected table " + tabName + " with column " + col + " by selector " + selector);
    ...
    public DBTable join(String tab1Name, String tab2Name, int tab1Col, int tab2Col) {
        log.debug("Joined table " + tab1Name + " and " + tab2Name + " with column " + tab1Col + " in table 1 and column " + tab2Col + " in table 2");
